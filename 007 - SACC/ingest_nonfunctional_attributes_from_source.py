#   Process all files in the repo to extract nonfunctional requirements from Source
#   Allow restart from failure

from datetime import datetime
from sacclib.source import extractNFRsFromSource, parseAndSaveNonFunctionalRequirementsFromSource
from sacclib.database import parseAndSaveLLMMetrics, getCompletedNonFunctionalRequirementsSource
from sacclib.utils import getResponseDictionary, loadConfig
from sacclib.files import linesInFile, getSourceFilesFromRepos

def AnalyseNFRsFromSource(llmserver="ollama", fname=None, doctype=None, verbose=False):
    
    result = extractNFRsFromSource(llmserver=llmserver, fname=fname, verbose=True)
    
    if (result):
        success = parseAndSaveLLMMetrics(llmserver=llmserver, label=f"Extracting NFRs from Source: {fname}", full_response=result, verbose=True)
        if (success):
        ...
        parseAndSaveNonFunctionalRequirementsFromSource(technologiesDict=technologiesDict, sourcedoc=sourcedoc, doctype=doctype)
    else:
...
for fname in todo:
    ...
    if (lines > MAXLINES):
        ...
        AnalyseNFRsFromSource(llmserver=llmserver, fname=fname, doctype=doctype, verbose=True)
